...
namespace RealmOne.Items
{
    public class VintageBlunderbuss: ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int d = 0; d < 40; d++)
            {
                Dust.NewDust(player.position, player.width, player.height, DustID.Smoke, 0f, 0f, 150, default(Color), 3f);
                Dust.NewDust(player.position, player.width, player.height, DustID.Torch, 0f, 0f, 150, default(Color), 3f);
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                Dust d = Dust.NewDustPerfect(Main.LocalPlayer.Center, DustID.Torch, speed * 10, Scale: 2.5f);; ;
